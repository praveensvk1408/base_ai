...
class PublishModelSchema(Schema):
    """This class will have the schema fields of \
        PublishModelSchemaValidation  File API"""

    result = fields.Str(strict=True, required=True)
    import_id = fields.Str(strict=True, required=True)


class PublishModelSchemaValidation:
    """This class having a method to invoke  \
    PublishModelSchemaValidation File schema validation"""

    def __init__(self):
        """Initialize a new instance of SchemaValidation"""
        self.publish_model = []  # taking an empty list to store the response

    def publish_model_schema(self, response):
        """This method will validate parameters in publish_model_schema"""
        try:  # using try except blocks for validation purpose
            self.publish_model.append(
                response
            )  # appending the response to the list
            # Deserialize and validate the input data using the schema
            PublishModelSchema(many=True).load(
                self.publish_model,
