...
class StopUploadInferenceResultSchema(Schema):
    """This class will have the schema fields of \
        StopUploadInferenceResultSchemaValidation  File API"""

    result = fields.Str(strict=True, required=True)


class StopUploadInferenceResultSchemaValidation:
    """This class having a method to invoke \
    StopUploadInferenceResultSchemaValidation File schema validation"""

    def __init__(self):
        """Initialize a new instance of SchemaValidation"""
        self.stop_upload_inference_result = (
            []
        )  # taking an empty list to store the response

    def stop_upload_inference_result_schema(self, response):
        """This method will validate parameters in
        start_upload_inference_result_schema"""
        try:  # using try except blocks for validation purpose
            self.stop_upload_inference_result.append(
                response
            )  # appending the response to the list
            # Deserialize and validate the input data using the schema
            StopUploadInferenceResultSchema(many=True).load(
                self.stop_upload_inference_result,
