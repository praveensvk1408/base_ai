...
class DeleteFirmwareSchema(Schema):
    """This class will have the schema fields of Delete Firmware"""

    result = fields.Str(strict=True, required=True)


class DeleteFirmwareSchemaValidation:
    """This class having a method to invoke
    Delete Firmware schema validation"""

    def __init__(self):
        """Initialize a new instance of SchemaValidation
        :param Delete Firmware: Will store response
        """
        self.delete_firmware = []  # taking an empty list to store the response

    def delete_firmware_positive_schema(self, response):
        """This method will validate parameters in Delete Firmware"""
        try:  # using try except blocks for validation purpose
            self.delete_firmware.append(
                response
            )  # appending the response to the list
            # Deserialize and validate the input data using the schema
            DeleteFirmwareSchema(many=True).load(
                self.delete_firmware, unknown=EXCLUDE
            )
