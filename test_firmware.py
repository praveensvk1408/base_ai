...
#     response = EdgeApp().Import_edge_app(
#         edge_app_package_id=file_id_app, 
#         app_name=APP_NAME,
#         app_version=APP_VER,
#     )
#     response_data = response.json()
...
#     response = EdgeApp().get_edge_app(APP_NAME, APP_VER)
#     response_data = response.json()
#     if response.status_code == 200:
...
#         )
#     assert(response.status_code == 200 and schema is True), "Unexpected response."
#     assert(EdgeApp().edge_app_status_check(APP_NAME, APP_VER) == "compiled"), "EdgeApp compilation failed."
...
# def test_create_deploy_configuration(SkipCheck):
...
#     response = Configuration().deploy_by_configuration(conf, dev)
#     response_data = response.json()
...
#         counter += 1
#         deploy_status = Configuration().deployment_status_check(dev) \
...
#         if counter == 3:
#             break
#     assert(deploy_status == "success"), "Deploy status is not success."
